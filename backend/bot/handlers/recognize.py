...
from keyboards.recipes_list import keyboard_recipes_builder, RecipesListCallback
...
async def get_recognize_text_cmd(message: Message, state: FSMContext):
    ...
    result = {"products": ["мёд", "чечевица", "масло"]}
...
async def get_approval_product_list(cb: CallbackQuery, callback_data: ProductListCallback, state: FSMContext):
    ...
    # {'recipes': [{'title': 'Чечевичная каша с мёдом', 'steps': ['Промойте чечевицу под проточной водой.', 'Вскипятите воду в кастрюле, добавьте чечевицу и варите на среднем огне 20–25 минут до мягкости чечевицы.', 'Слейте лишнюю воду, добавьте мёд по вкусу и перемешайте.']}, {'title': 'Салат из чечевицы с мёдом и зеленью', 'steps': ['Отварите чечевицу в кипящей воде до готовности (около 20 минут).', 'Дайте чечевице остыть, затем добавьте немного мёда и мелко нарезанную зелень (укроп, петрушку).', 'Добавьте немного масла, перемешайте и подавайте.']}, {'title': 'Запечённая чечевица с мёдом', 'steps': ['Разогрейте духовку до 180 градусов Цельсия.', 'Выложите чечевицу в форму для запекания, добавьте мёд и масло, перемешайте.', 'Запекайте в духовке около 30 минут до золотистой корочки.']}]}
...
async def get_approval_product_list(cb: CallbackQuery, callback_data: RecipesListCallback, state: FSMContext):
